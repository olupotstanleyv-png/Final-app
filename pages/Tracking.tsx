...
import { MapPin, Truck, CheckCircle, User, Phone, Navigation, MessageCircle, Clock, Map as MapIcon, ShoppingBag, Utensils, Home } from 'lucide-react';
import { Order, DeliveryAgent } from '../types';
import { fetchOrders, fetchAgents } from '../services/menuRepository';
...
const Tracking: React.FC = () => {
    ...
    // Live Tracking State
    // Start Agent slightly away from destination for demo purposes
        ...
        const loadData = async () => {
            ...
            if (found) {
                ...
                if (found.deliveryAgentId) {
                    ...
                    if (da) {
                        setAgent(da);
                        // If agent has real coords, use them, else default to start pos
                        // For demo smoothness we keep local state mostly, but in real app we'd sync
                        // setAgentPos({ lat: da.currentLat, lng: da.currentLng });
                    }
                }
            }
            setLoading(false);
    ...
    const angle = Math.atan2(destination.lat - agentPos.lat, destination.lng - agentPos.lng) * 180 / Math.PI; // simple bearing approx
                    ...
                    {agent ? (
                        <div className="bg-white p-5 rounded-2xl border border-stone-200 shadow-sm">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <button className="bg-stone-50 hover:bg-stone-100 text-stone-700 py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition border border-stone-200">
                                    <MessageCircle size={14} className="text-stone-400"/> Chat
                                </button>
                            </div>
                        </div>
                    ) : (
